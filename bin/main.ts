...
new NginxIngressConstruct().buildAsync(app, "nginx").catch(() => {
  logger.info(
    "NGINX Ingress pattern is not setup due to missing secrets for ArgoCD admin pwd."
  );
});
//-------------------------------------------
...
new MultiRegionConstruct().buildAsync(app, "multi-region").catch((error) => {
  logger.info(
    "Multi region pattern is not setup due to missing secrets: ",
    error
  );
});
...
new PipelineMultiEnvMonitoring().buildAsync(app).catch(() => {
  logger.info(
    "Multi Account Monitoring pattern is not setup due to missing secrets for GitHub \
...
if (account) {
  ...
  logger.info(
    "Valid AWS credentials are required to synthesize pipeline stack. Please run 'aws configure'"
  );
}
...
new DynatraceOperatorConstruct()
  .buildAsync(app, "dynatrace-operator")
  .catch(() => {
    logger.info(
      "Dynatrace pattern is not setup due to missing secrets for dynatrace-tokens."
    );
  });
